...
app.get('/load_pmx', async (request, reply) => {
  const pmxResponse = await axios.get('https://github.com/TakuKobayashi/research-examples/raw/master/mmd-parser-sample/sample_model/PronamaChan.pmx', { responseType: 'arraybuffer' })
...
app.get('/load_vmd', async (request, reply) => {
  const vmdResponse = await axios.get('https://github.com/TakuKobayashi/research-examples/raw/master/mmd-parser-sample/sample_model/run.vmd', { responseType: 'arraybuffer' })
